...
const PromptBuilder = ({ authDetails }) => {
  ...
  const [routeParams, setRouteParams] = useState({
    ...
    display_name:
      decodeURI(router.query?.display_name) ||
      decodeURIComponent(router.query?.id?.[0] || "0"),
  });
      ...
      submitPromptJob(
        ...
        (data) => {
          if (data?.errorMessage) {
            ...
            router.push(`/run-status?newJob=${g}`, undefined, {
              shallow: true,
            });
          }
        },
  ...
  const handleTableRowSubmit = (filterOptions, extras) => {
    ...
    try {
      ...
      submitRowCountRequest(routeParams.table, filterOptions, extras).then(
        (data) => {
          if (data?.errorMessage) {
  ...
  useEffect(() => {
    if (routeParams.table === "0") {
      ...
      getTableFilters(routeParams.table)
        .then((data) => {
          if (data?.errorMessage) {
            setErrorMessage(data.errorMessage);
            setShowUserMessage(true);
    ...
    <>
      ...
      {showUserMessage && (
        <Box>
          <Alert
            ...
            emphasis='critical'
            actions={
              <Button design='inline' onClick={handleCloseError} text='Close' />
            }
          />
        </Box>
      )}
      ...
      {!isLoading && !showTableSelect && (
        <>
          <Box>
            <text.h1
              text={routeParams.display_name || "missing"}
              as='heading'
            />
            <TwoColumnLayout>
              <Box>
                      ...
                      >
                        <MessageOverlay onEventBehind={handleTableRowSubmit}>
                          ...
                          >
                            <text.label
                              as='label'
                              text='Getting number of transcripts based on your selections'
                            />
                            <Spinner size='md' />
                          </Box>
                        </MessageOverlay>
