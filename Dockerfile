...
# RUN openssl req -new -key /app/server.origKey -out /app/server.csr -subj "/C=US/ST=Arizona/L=Scottsdale/O=Go Daddy/OU=Cloud Platform/CN=*.frontdoor.gdcorp.tools/emailAddress=mswaagman@godaddy.com" -passin pass:server
...
# # Create final image which copies from cached base and adds remaining application files.
# FROM 764525110978.dkr.ecr.us-west-2.amazonaws.com/alpine-node:18-alpine-3.17
# USER root

# # Copy from the cached base layer
# COPY --from=fd-base /app /app
