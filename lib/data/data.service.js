...
async function fetchTestResponse() {
  // Mock data emuldation for promise response  
  const mockBody = {
    "statusCode": 200,
    "body": "{\"hit_count\": 1204464, \"messaging_hit_count\": 873308, \"speech_hit_count\": 331156, \"total_count\": 2427605, \"messaging_total_count\": 1780873, \"speech_total_count\": 646732, \"hit_percentage\": 49.6153204495789, \"messaging_hit_percentage\": 49.03819643511918, \"speech_hit_percentage\": 51.204517481738954}"
  }
...
async function getLexicalQueryHits(query) {
  ...
  try {
    ...
    // using for dev   const response = await fetchTestResponse();
    if (response?.body) {
      let data = JSON.parse(response.body);
      let groups = Object.entries(data).map(([key, value]) => key.split('_')[0]).filter((group) => group != 'total');
      let sections = Object.entries(data).map(([key, value]) => {
      ...
      groups = groups.map((group) => {
        return {
          ...
          count: sections.filter((section) => section.name === `${group}_count`)[0]?.value || 0,
          total: sections.filter((section) => section.name === `${group}_total_count`)[0]?.value || 0,
          percentage: sections.filter((section) => section.name === `${group}_percentage`)[0]?.value || 0
        }
      })
      return groups;
    } else {
...
async function deleteLexicalQuery({ queryId }) {
  const options = {
    method: 'POST',
    headers: getHeaders(),
...
async function submitLexicalQuery({ query_name, query, description }) {
  const body = {
    query_name,
    description,
    query,
...
/**
 *  * @param {Object} formData {parent_run_id, new_run_id, model, prompt, count, provider}
 * 
 */
