...
const mockResponseBody = "{\"hit_count\": 819219, \"messaging_hit_count\": 631296, \"speech_hit_count\": 187923, \"total_count\": 1672107, \"messaging_total_count\": 1307684, \"speech_total_count\": 364423, \"hit_percentage\": 48.99321634321249, \"messaging_hit_percentage\": 48.27588316443422, \"speech_hit_percentage\": 51.567272098632635}";
// Building the model from the object
const queryHits = [
  {
    name: 'Hits',
    id: 'hit',
...
async function getLexicalQueryHits(query) {
  ...
  try {
    const data = await secureProxyPost('get-lexical-query-hits', {}, options);
    if (data?.body) {
      return JSON.parse(data.body);
    } else {
      alert('No data. going to return mock data.');
      let mockData = JSON.parse(mockResponseBody);
      mockData.keys.forEach((key) => {
        // Determine id based on data first word
        let id = key.split(' ')[0].toLowerCase();
        let field = key.split('_').pop().toLowerCase();
        console.log(id, field);
        queryHits.find((item) => item.id == id).data = {
          [field]: mockData[key][field]
        };
      })
      return queryHits;
    }
