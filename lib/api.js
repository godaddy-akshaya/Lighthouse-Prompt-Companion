...
async function submitRowCountRequest2(table, filterOptions, extras) {
    // Remove extra properties from filterOptions
    let cleanFilterOptions = filterOptions.filter((option) => option.has_been_modified).map((option) => {
        return {
            column_name: option.column_name,
    ...
    try {
        const data = await secureProxyPost('row-count-2', {}, options);
        return JSON.parse(data.body);
    } catch (error) {
        return { error: error };
    }
...
module.exports = {
    getTables, getTableFilters, submitRowCountRequest2, submitRowCountRequest, submitPromptJob, submitSummaryPromptJob, getStatus, cancelJob, getResultsByRunId, getSummaryResultsByRunId
}
