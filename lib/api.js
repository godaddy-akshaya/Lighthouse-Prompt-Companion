...
async function validateLexicalQuery(query) {
    ...
    try {
        ...
        return new Error({ 'error': 'Error validating lexical query' });
    }
}
async function getAllLexicalQueries() {
    ...
    try {
        const response = await secureProxyFetch('submit-lexical-query', {}, options);
        const data = JSON.parse(response?.body || response);
        return data;
    } catch (error) {
        console.error('Error getting lexical queries', error);
        return new Error({ 'error': 'Error getting lexical queries' });
    }
}

async function updateLexicalQuery(query) { }

async function deleteLexicalQuery(query) { }

async function submitLexicalQuery({ query_name, query }) {
    const options = {
        method: 'POST',
        headers: getHeaders(),
        body: JSON.stringify({ query_name, query, action: 'insert' })
    };
    try {
