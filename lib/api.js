...
async function secureProxyFetch(endpoint, params = {}, options = {}) {
    ...
    let url = '/aws/secure-data/' + endpoint;
...
async function cancelJob(job) {
    ...
    try {
        const data = await secureProxyFetch('cancel-job', {}, options);
...
async function submitSummaryPromptJob(formData) {
    ...
    try {
        const data = await secureProxyFetch('submit-summary-job', {}, options);
...
async function submitPromptJob(table, job, filterOptions, extras) {
    ...
    try {
        const data = await secureProxyFetch('submit-job', {}, options);
        if (data?.errorMessage) throw new Error(data.errorMessage);
        return JSON.parse(data.body);
    } catch (error) {
        return { error: error };
...
async function submitRowCountRequest(table, filterOptions, extras) {
    ...
    try {
        const data = await secureProxyFetch('table-data-row-count', {}, options);
        return JSON.parse(data.body);
    } catch (error) {
        return { error: error };
    }
