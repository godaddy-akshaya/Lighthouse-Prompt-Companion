...
import { getAuthKey } from '@godaddy/gasket-auth/lib/utils'
...
async function authFetch(url, options) {
    ...
    console.log(authKey);
    const headers = getHeaders({ Authorization: `sso-jwt ${authKey}` });
    console.log(headers);
...
async function fetchData(endpoint, params = {}, options = {}) {
    ...
    const fetchType = env == 'production' ? authFetch(url, options) : fetch(url, options);
...
async function submitPromptJob(table, job, filterOptions, extras) {
    console.log('submitPromptJob', table, job, filterOptions, extras);
