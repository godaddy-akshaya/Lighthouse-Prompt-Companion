...
class FilterParamsMgmtService {
    ...
    async saveFilterOptions(fileNameAndData) {
        ...
        const data = await secureProxy('gdlh-save-interaction-ids', {}, options, 'POST');
        return data;
    }
    async getFilterValues(filename) {
        ...
        const data = await secureProxy('gdlh-get-interction-ids', params, { headers: getHeaders() });
        return JSON.parse(data.body);

    }
    async getFilterOptions() {
        const data = await secureProxy('gdlh-get-interction-ids', {}, { headers: getHeaders() });
        return JSON.parse(data.body);
    }
}
